...
class SetNewPasswordByTokenAction extends \DoEveryApp\Action\AbstractAction
{
    ...
    public function run(): \Psr\Http\Message\ResponseInterface
    {
        ...
        try {
            ...
            $existing = \DoEveryApp\Entity\Worker::getRepository()
                                                 ->findOneByPasswordResetToken(token: $token)
            ;
            if (false === $existing instanceof \DoEveryApp\Entity\Worker) {
                throw new \RuntimeException(message: 'user not found by token');
            }
            if (
                \Carbon\Carbon::createFromDate(year: $started)
                              ->addMinutes(value: 10) < \Carbon\Carbon::now()
            ) {
        ...
        try {
            ...
            if ($data[static::FORM_FIELD_PASSWORD] !== $data[static::FORM_FIELD_PASSWORD_CONFIRM]) {
                $this
                    ->getErrorStore()
                    ->addError(key: 'password', message: \DoEveryApp\Util\DependencyContainer::getInstance()
                                                                                             ->getTranslator()
            ...
            if (true === $existing->getPasskeyCredential() instanceof \DoEveryApp\Entity\Worker\Credential) {
                $existing
                    ->getPasskeyCredential()::getRepository()
                    ->delete($existing->getPasskeyCredential())
                ;
            }
            if (true === $existing->getPasswordCredential() instanceof \DoEveryApp\Entity\Worker\Credential) {
                $existing
                    ->getPasswordCredential()::getRepository()
                    ->delete($existing->getPasswordCredential())
                ;
            }
            ...
            return $this->redirect(to: LoginAction::getRoute());
