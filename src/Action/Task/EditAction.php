    ...
    \DoEveryApp\Action\AbstractAction
{
    ...
    public function run(): \Psr\Http\Message\ResponseInterface
    {
            ...
            return $this->render(script: 'action/task/edit',
                                 data  : [
                                             ...
                                             'data' => [
                                                 ...
                                                 static::FORM_FIELD_REMIND_DATE          => $task->getRemindDate()?->format(format: 'Y-m-d H:i:s'),
                                                 static::FORM_FIELD_DUE_DATE             => $task->getDueDate()?->format(format: 'Y-m-d H:i:s'),
                                             ],
                                         ]);
        }
        $data = [];
        try {
            ...
            $this->handleEditOrClone(task: $task, data: $data);
            $task::getRepository()
                 ->update(entity: $task)
            ;
