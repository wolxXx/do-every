...
class DueTest extends \DoEveryAppTest\TestBase
{
    ...
    {
        $testNow = '2024-01-10 12:00:00';
        \Carbon\Carbon::setTestNow(testNow: $testNow);
        $this->assertSame(expected: $testNow, actual: \Carbon\Carbon::getTestNow()->format(format: 'Y-m-d H:i:s'));
        $this->assertSame(expected: $testNow, actual: \Carbon\Carbon::now()->format(format: 'Y-m-d H:i:s'));
    ...
    public static function getTestDataProvider(): array
    {
        $nowDueTask = new \DoEveryApp\Entity\Task()
            ...
            ->setType(\DoEveryApp\Definition\TaskType::RELATIVE)
